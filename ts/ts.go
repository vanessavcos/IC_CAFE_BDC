...
func (m *mongoDB) Read(ctx context.Context, table string, key string, fields []string) (map[string][]byte, error) {
    ...
    doc["metadata"] = []byte(fmt.Sprintf(`{"type":"%s","coordinates":%v}`, result.Metadata.Type, result.Metadata.Coordinates))
...
func parseCoordinates(value []byte) []float64 {
    ...
    if err := bson.Unmarshal(value, &metadata); err != nil {
        return nil
    }
...
func (m *mongoDB) Update(ctx context.Context, table string, key string, values map[string][]byte) error {
    ...
    for k, v := range values {
        ...
        case "metadata":
            var metadata Metadata
            if err := bson.Unmarshal(v, &metadata); err != nil {
                return fmt.Errorf("Update error: %s", err.Error())
            }
...
func (c tsCreator) Create(p *properties.Properties) (ycsb.DB, error) {
	...
	if err := db.CreateCollection(ctx, "timeSeriesCollection", opts); err != nil {
...
func init() {
	ycsb.RegisterDBCreator("ts", tsCreator{})
}
